...
int blitzed_model_predict(const int8_t* input, int input_len,
    ...
    for (int j = 0; j < LAYER1_OUTPUT_SIZE; j++) {
        ...
        for (int i = 0; i < LAYER1_INPUT_SIZE && i < input_len; i++) {
            ...
            // to safely handle up to 20 * 127 * 127 = 322580 without overflow
    ...
    // Layer 2: Dense(32, 5) — output layer
...
int8_t blitzed_quantize_input(float value) {
    // Features are already normalized to [0, 1] by extract_touch_features(),
    // so INPUT_NORM_FACTOR = 1.0 and we skip the division step.
    float normalized = value / INPUT_NORM_FACTOR;
