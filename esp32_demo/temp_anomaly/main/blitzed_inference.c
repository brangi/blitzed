...
int blitzed_model_predict(const int8_t* input, int input_len, int8_t* output, int output_len) {
    ...
    // Layer 2: Dense(16, 4) - Output layer (4 classes)
...
int8_t blitzed_quantize_input(float temp_celsius) {
    // Normalize to [0, 1] range based on the training normalization factor (INPUT_MAX = 120.0)
    // This mirrors the Python training: X_normalized = X / 120.0
    float normalized = temp_celsius / INPUT_MAX;

    // Clamp to valid normalized range
    if (normalized > 1.0f) normalized = 1.0f;
    if (normalized < (INPUT_MIN / INPUT_MAX)) normalized = INPUT_MIN / INPUT_MAX;
