...
impl RaspberryPiTarget {
    ...
    pub fn estimate_inference_latency(&self, model: &Model) -> Result<RpiInferenceEstimate> {
        ...
        Ok(RpiInferenceEstimate {
            ...
            estimated_fps: if final_latency > 0 { 1_000_000 / final_latency } else { 0 },
...
impl HardwareTarget for RaspberryPiTarget {
    ...
    fn optimization_strategy(&self) -> OptimizationStrategy {
        ...
        OptimizationStrategy {
            ...
            enable_pruning,
            target_precision,
            memory_optimization: matches!(self.specs.variant, RpiVariant::PiZero | RpiVariant::PiZero2),
...
impl RpiInferenceEstimate {
    ...
    pub fn optimization_recommendations(&self) -> Vec<String> {
        ...
        if self.can_use_gpu_acceleration {
            recommendations.push("GPU acceleration available - consider OpenGL/Vulkan compute".to_string());
        }
        ...
        if self.latency_us > 100_000 { // > 100ms
...
mod tests {
    ...
    fn test_memory_constraints() {
        ...
        println!("Model size: {} bytes, Pi Zero memory limit: {} bytes", 
                model_size, pi_zero.constraints().memory_limit);
    ...
    fn test_hardware_constraints() {
        ...
        assert!(constraints.accelerators.contains(&"VideoCore GPU".to_string()));
